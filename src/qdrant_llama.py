...
def connect_to_qdrant():
    """Conecta con la base de datos Qdrant."""
    return qdrant_client.QdrantClient(path="./qdrant_db") 
 
def create_or_get_collection(client, collection_name="pdf_chunks", vector_size=384):
    """Crea o accede a la colección en Qdrant."""
...
def store_embeddings_in_qdrant(client, collection_name, embeddings, chunks):
    """Almacena los embeddings en Qdrant."""
    points = [
        PointStruct(id=i, vector=embedding.tolist(), payload={"chunk": chunk})
        for i, (embedding, chunk) in enumerate(zip(embeddings, chunks))
    ]
    client.upsert(collection_name=collection_name, points=points)
 

def search_in_qdrant(client, collection_name, query_embedding, top_k=7):
...
def generate_response(results, question, model_name='llama3.2'):
    ...
    texts = [hit.payload['chunk'] for hit in results]
    ...
    try:
        ...
        adjusted_response = adjust_tense(response)
...
def process_pdf():
    ...
    collection_name = create_or_get_collection(client, vector_size=len(embeddings[0]))
    store_embeddings_in_qdrant(client, collection_name, embeddings, chunks)
...
def ask_question():
    ...
    results = search_in_qdrant(client, collection_name, query_embedding, top_k=7)
